...
export default async function handler(req, res) {
  ...
  try {
    ...
    let meetLink = `https://meet.google.com/${Math.random().toString(36).substring(2, 15)}`
    ...
    if (session.accessToken) {
      try {
        ...
        if (calendarResult.success) {
          meetLink = calendarResult.meetLink || meetLink
          googleEventId = calendarResult.eventId
        }
      } catch (error) {
        console.error('Google Calendar integration failed, using fallback:', error)
      }
    }
    ...
    res.status(201).json({ 
      ...
      class: newClass,
      googleCalendarIntegrated: !!googleEventId
    })
  } catch (error) {
